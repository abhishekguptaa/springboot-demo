pipeline{
    ...
    stages{
     stage ('Build'){
        ...
        stage('Nexus'){
            steps{
                nexusArtifactUploader artifacts: [[artifactId: 'spring-boot-starter-parent', classifier: '', file: '/var/lib/jenkins/workspace/Demo1/target/springboot-0.0.1-SNAPSHOT.war', type: 'war']], credentialsId: 'Nexus', groupId: 'org.springframework.boot', nexusUrl: '159.65.148.159:8081', nexusVersion: 'nexus3', protocol: 'http', repository: 'Abhishek_demo', version: '1.1'
            }
        }
        stage('Sonar') 
       {environment {
        ...
        stage("Deploy to Tomcat"){
            steps{
                sh "sudo rm -rf /home/ec2-user/apache-tomcat-9.0.31/webapps/springboot-0.0.1-SNAPSHOT.war"
                sh "sudo cp /var/lib/jenkins/workspace/Demo1/target/springboot-0.0.1-SNAPSHOT.war /home/ec2-user/apache-tomcat-9.0.31/webapps/"
            }
        }
    /*
